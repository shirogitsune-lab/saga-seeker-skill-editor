...
Saga & SeekerのキャラクターシートHTMLに含まれるスキルと性格キーワードを、安全に編集するためのWindows向けGUIアプリケーションです。
...
4. 「HTMLファイルを開く」からキャラクターシートを選び、スキルまたは性格キーワードを編集します。
...
`SagaSeekerSkillEditor.spec`はEXEアイコンを設定し、onefile版へ`gui/styles/*.qss`と性格キーワードカタログを同梱します。ビルドスクリプトはonedir版にも同じリソースを含めます。PyInstallerのonefile実行時に使用される`_MEIPASS`を含め、リソースパスの解決は一か所へ集約しています。
