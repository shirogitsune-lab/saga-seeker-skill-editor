...
- `onefile`: エディター単体のEXEです。1ファイルだけで使用したい場合に選べます。利用ガイドはReleaseの`SagaSeekerSkillEditor-v2.2.0-guide.html`を別途ダウンロードできます。
...
互換用の「魅力」は画面に表示せず、既存値を保持します。新規シートでは`E`です。画像はPNG、JPEG、WebPを選択し、正方形切抜き後に512×512・WebP品質90で保存します。シートを読み込むと、既存画像は安全検査後に自動表示されます。表示に失敗しても他の項目は引き続き編集でき、「プレビューを再読込」で再試行できます。
...
各スキル枠の「デフォルトスキルを選択…」から、同梱されたゲーム内96件のカタログを名前・説明文で検索し、「肉体」「精神」「社会」で絞り込めます。選択直後は未確定で、名前と説明を編集できます。保存時に名前と説明が改行正規化後もカタログ正本と一致していれば、`id`、`name`、`description`、`type`、`key`を正規デフォルトとしてまとめて確定します。どちらかを変更したまま保存すると、未確定の選択はオリジナルスキルとして保存されます。`key`を名前などから推測・補正することはありません。キャンセルだけでは変更扱いにならず、「リセット」で読込み時の内容へ戻せます。

スキルの削除や、保存済みの保護されたデフォルトスキルをオリジナルへ置き換える操作は「高度な操作」に分離されています。後者は従来どおり二重確認が必要です。未保存のデフォルト候補を編集してオリジナルとして保存する経路とは区別されます。リスト途中のスキルを削除する場合は、位置対応を維持するため空スキルへ置き換えます。末尾のスキルを削除する場合は、ゲームが自動追加に使用できる未使用枠へ戻します。
...
「表示 > 背景」から「背景なし / 背景あり」を切り替えられます。背景ありでは全5タブの背後に強くぼかした庭園写真を表示し、カードと入力欄の可読性を保ちます。ハイコントラストでは設定値にかかわらず背景画像を表示しません。
