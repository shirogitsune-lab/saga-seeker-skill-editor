...
「性格キーワード」タブでは、ゲーム内に用意された150件から最大6件を選択できます。「力」「知恵」「富」「愛」「法」の系統と、「美徳」「中庸」「悪徳」の傾向を独立して絞り込めます。検索欄へ名前の一部を入力すると全系統から一致する候補を表示し、系統・傾向フィルターと組み合わせることもできます。候補を設定枠へドラッグして追加でき、設定済みキーワードはドラッグまたは「上へ移動」「下へ移動」で並び替えられます。途中のキーワードを削除すると後続項目を自動的に前へ詰めます。各項目のID、名前、系統、カルマをまとめて設定し、オリジナルの性格キーワードは作成しません。同じキーワードの重複や途中の未設定枠がある場合は、その場でエラーを表示して保存を止めます。
